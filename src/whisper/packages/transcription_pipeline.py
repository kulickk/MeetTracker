...
from src.whisper.packages.utils import measure_time
    ...
    def run(self, file_path: str, **kwargs) -> str:
        ...
        def prepare_json(segments: List) -> List[Dict[str, Any]]:
            with ThreadPoolExecutor() as executor:
                # Обработка каждого сегмента с использованием prepare_segment
                result = list(executor.map(prepare_segment, segments))
            return result
        ...
        # print(f'JSONN:{json_string.decode("utf-8")}')
